...
async def filters(text):
    ...
    if "task env edit " in text:
        ## 脚本名
        script = re.search(r'/tools/(.*?)\b ', text, re.M | re.I)[1]
        ...
        # 超级无线 · 店铺抽奖
        if script == 'jd_lzclient.js' or script == 'm_jd_wx_luckDraw.js' or script == 'jd_drawCenter.js':
            text += ' -c 1,2,3'

        # 超级无线 · 加购有礼
        elif script == 'm_jd_wx_addCart.js':
            text += ' -c 1,2,3 -l 1'
        ...
        # 超级无线 · 通用游戏
        elif script == 'jd_dadoudou.js':
            text += ' -c 1,2,3'

        # 店铺签到
        elif script == 'jd_dpqd.js':
            text = text
